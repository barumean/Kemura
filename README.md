...
### 문자 인코딩 (SHIFT-JIS / UTF-8)

era 게임의 ERB·CSV 는 대부분 **SHIFT-JIS**(코드페이지 932)로 작성돼 있습니다.
UTF-8 로 저장된 게임도 있습니다. **파일마다 자동으로 판정하므로 둘 다 그대로
동작합니다.** 한 게임 안에 두 인코딩이 섞여 있어도 됩니다.

판정 순서: BOM → 엄격한 UTF-8 디코드 시도 → 실패하면 SHIFT-JIS.
일본어 SHIFT-JIS 는 거의 확실하게 유효한 UTF-8 이 아니고(2바이트 문자의 뒷바이트가
`0x40~0x7E` 에 흔히 오는데 UTF-8 계속 바이트는 `0x80~0xBF` 여야 합니다), UTF-8
파일은 항상 엄격한 디코드를 통과하므로 서로 오판하지 않습니다.

> **글자가 `?`(U+FFFD)로 깨져 보이면** — 게임 파일을 다시 저장할 필요가 없습니다.
> `U+FFFD` 는 디코더가 만들어내는 문자이므로 원인은 항상 **읽는 쪽**입니다.
> 로그에 `[EraEncoding]` 줄이 있는지 확인해주세요. 이 줄이 없거나
> `SHIFT-JIS(932) 를 사용할 수 없습니다` 경고가 있으면 알려주세요.

세이브 파일은 **읽기는 판정, 쓰기는 UTF-8** 입니다. PC판 Emuera 가 만든 SHIFT-JIS
세이브를 읽을 수 있고, 이 앱이 만든 세이브는 한글 이름도 보존됩니다(932 는 한글을
표현할 수 없어 `?` 로 소실됩니다).
